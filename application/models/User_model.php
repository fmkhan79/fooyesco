...
class User_model extends Base_model
{
    ...
    public function is_guest($user_id) {
        $this->db->select('is_guest');
        $this->db->from('users');
        $this->db->where('id', $user_id); // Assuming 'id' is the primary key
        $query = $this->db->get();
...
public function check_user_role($user_id) {
    ...
    public function update_profile()
    {
        ...
        } elseif (get_user_role('user_role', $this->session->userdata('user_id')) == "cook") {
            return $this->update_cook_profile();
        } else {
    ...
    // UPDATE DRIVER PROFILE
    public function update_cook_profile()
    {
        $logged_in_user_id = $this->session->userdata('user_id');
        $previous_data = $this->get_user_by_id($logged_in_user_id);
        $email = required(sanitize($this->input->post('email')));
