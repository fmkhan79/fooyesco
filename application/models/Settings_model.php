...
class Settings_model extends Base_model
{
    ...
    public function update_delivery_settings()
    {
        ...
        // $maximum_range = !empty(sanitize($this->input->post('maximum_range'))) ? sanitize($this->input->post('maximum_range')) : 0;
        // $this->db->where('key', 'maximum_range');
        // $this->db->update('delivery_settings', ['value' => $maximum_range]);
    
        // $free_range = !empty(sanitize($this->input->post('free_range'))) ? sanitize($this->input->post('free_range')) : 0;
        // $this->db->where('key', 'free_range');
        // $this->db->update('delivery_settings', ['value' => $free_range]);
        return true;
    }
