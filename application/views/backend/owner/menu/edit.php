...
<section class="content">
   ...
   <div class="form-group">                               
    ...
    <?php if ($menu_data['subcategory_id'] == null) : ?>
        <option value="" selected disabled>Select Sub Category</option>
    <?php endif; ?>
        <?php foreach ($subcategories as $sub) : ?>
            <option value="<?php echo $sub['id']; ?>"
                <?php echo ($menu_data['sub_category_id'] == $sub['id']) ? 'selected' : ''; ?>>
                            ...
                            <div class="col-xl-6">
                                <form action="<?php echo site_url('menu/update'); ?>" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="row">
                                            <div class="col">
                                                <button type="button" class="btn btn-info mt-4 float-right" onclick="addRow('tabular-input')"><?php echo get_phrase('add_new_row'); ?></button>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label for="warning"><?php echo get_phrase("menu_warning"); ?></label>
                                        <table id="tabular-input-warning-field" class="data-table data-table-horizontal data-table-highlight">
                                            <tbody>
                                                <?php $warnings = json_decode($menu_data['warnings'], true);
                                                if (is_array($warnings) && count($warnings) > 0) :
                                                    foreach ($warnings as $key => $warning) : ?>
                                                        <tr>
                                                            <td><input type="text" class="form-control" name="warning[]" placeholder="<?php echo "E.g : " . get_phrase("contains_dairy"); ?>" value="<?php echo sanitize($warning); ?>"></td>
                                                            <td><a type="button" class="btn btn-default float-right" value="Delete" onclick="deleteRowForWarning(this)"><i class="fas fa-trash"></i></a></td>
                                                        </tr>
                                                    <?php endforeach; ?>
                                                <?php else : ?>
                                                    <tr>
                                                        <td><input type="text" class="form-control" name="warning[]" placeholder="<?php echo "E.g : " . get_phrase("contains_dairy"); ?>"></td>
                                                        <td><a type="button" class="btn btn-default float-right" value="Delete" onclick="deleteRowForWarning(this)"><i class="fas fa-trash"></i></a></td>
                                                    </tr>
                                                <?php endif; ?>
                                            </tbody>
                                        ...
                                        <div class="row">
                                            <div class="col-6">
                                                <button type="submit" class="btn btn-primary mt-4 float-left"><?php echo get_phrase('update_menu_details'); ?></button>
                                            </div>
                                            <div class="col-6">
                                                <button type="button" class="btn btn-info float-right mt-4" onclick="addRow('tabular-input-warning-field')"><?php echo get_phrase('add_new_row'); ?></button>
                                            </div>
                                        </div>
                                    </div>
                                </form>
