...
{
    $data['customer_id'] = 1; // POS default
...
{
    // Get grand total from cart (POS only)
    $data['amount_to_pay'] = $this->cart_model->get_grand_total('pos'); 
    $data['amount_paid'] = 0;
    $data['payment_method'] = "cash_on_delivery";
    $data['data'] = json_encode([]);
    $data['created_at'] = time(); // timestamp

    // Create order and get order code
    $order_code = $this->order_model->confirm(null, 'pos');
    $data['order_code'] = $order_code;

    // Insert payment
    $this->db->insert('payment', $data);

    // Prepare order updater
    $billing_data = $this->session->userdata('billing') ?: [];
    $json_billing_data = json_encode($billing_data);

    $updater = [
        'order_type' => 'pos',  // POS order type
        'grand_total' => $data['amount_to_pay'],
        'billing' => $json_billing_data,
        'driver_id' => null
    ];

    // Update order table with billing and total
